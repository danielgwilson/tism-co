...
async function getTwitterProfileData(handle: string) {
  ...
  const profileUrl =
    'https://api.apify.com/v2/acts/kaitoeasyapi~premium-twitter-user-scraper-pay-per-result/run-sync-get-dataset-items';
  const profileInput = {
  ...
  try {
    ...
    if (!profileResponse.ok) {
      ...
      console.error(
        `[APIFY] Profile API Error ${profileResponse.status}:`,
        errorText
      );
    ...
    if (!tweetsResponse.ok) {
      ...
      console.error(
        `[APIFY] Tweets API Error ${tweetsResponse.status}:`,
        errorText
      );
    ...
    if (!profileData || profileData.length === 0) {
      throw new Error(
        `No profile data found for @${handle}. Profile may be private or non-existent.`
      );
    }
    ...
    const tweets =
      tweetsData
        ?.map((tweet: { text?: string }) => tweet.text)
        .filter(Boolean) || [];
    ...
    const combinedProfileData = {
      ...
      stats: {
        followers:
          userProfile.relationship_counts?.followers?.toString() || '0',
        following:
          userProfile.relationship_counts?.following?.toString() || '0',
    ...
    console.log(
      `[APIFY] Combined data for @${combinedProfileData.handle}: ${tweets.length} tweets, ${combinedProfileData.stats.followers} followers`
    );
    ...
    throw new Error(
      `Failed to fetch Twitter data for @${handle}: ${
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `[API] Fetching Twitter data for @${sanitizedHandle} via Apify`
    );
    ...
    try {
      ...
      console.error(
        `[API] Apify scraping failed for @${sanitizedHandle}:`,
        error
      );
      return NextResponse.json(
        {
          ...
          suggestion:
            'Please verify the handle exists and is public, then try again.',
        },
        { status: 503 }
      );
    }
